...
//static LPDIRECTINPUTDEVICE81	pGamePad1[GAMEPADMAX] = { NULL,NULL,NULL,NULL };// パッドデバイス

//static DWORD	padState1[GAMEPADMAX];	// パッド情報（複数対応）
//static DWORD	padTrigger1[GAMEPADMAX];
//static int		padCount1 = 0;			// 検出したパッドの数
//
...
//			return false; // モードの設定に失敗
		...
		//// X軸の範囲を設定
		//diprg.diph.dwObj = DIJOFS_X;
		//pGamePad1[i]->SetProperty(DIPROP_RANGE, &diprg.diph);
		//// Y軸の範囲を設定
		//diprg.diph.dwObj = DIJOFS_Y;
		//pGamePad1[i]->SetProperty(DIPROP_RANGE, &diprg.diph);
		...
		////X軸の無効ゾーンを設定
		//dipdw.diph.dwObj = DIJOFS_X;
		//pGamePad1[i]->SetProperty(DIPROP_DEADZONE, &dipdw.diph);
		////Y軸の無効ゾーンを設定
		//dipdw.diph.dwObj = DIJOFS_Y;
		//pGamePad1[i]->SetProperty(DIPROP_DEADZONE, &dipdw.diph);
...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//lastPadState1 = padState1[i];

		padState[i] = 0x00000000l;	// 初期化
		//padState1[i] = 0x00000000l;	// 初期化
		...
		//result1 = pGamePad1[i]->Poll();	// ジョイスティックにポールをかける
		//if (FAILED(result)) {
		//	result1 = pGamePad1[i]->Acquire();
		//	while (result == DIERR_INPUTLOST)
		//		result = pGamePad1[i]->Acquire();
		//}

		//result1 = pGamePad1[i]->GetDeviceState(sizeof(DIJOYSTATE), &dijs1);	// デバイス状態を読み取る
		//if (result1 == DIERR_INPUTLOST || result1 == DIERR_NOTACQUIRED) {
		//	result1 = pGamePad1[i]->Acquire();
		//	while (result1 == DIERR_INPUTLOST)
		//		result1 = pGamePad1[i]->Acquire();
		//}
		...
		if (dijs.lZ < 0)					padState[i] |= BUTTON_RUP;
		//* y-axis (backward)
		if (dijs.lZ > 0)					padState[i] |= BUTTON_RDOWN;
		//* x-axis (left)
		if (dijs.lRz < 0)					padState[i] |= BUTTON_RLEFT;
		//* x-axis (right)
		if (dijs.lRz > 0)					padState[i] |= BUTTON_RRIGHT;
		...
		////* Ｍボタン
		//if (dijs.rgbButtons[10] & 0x80)	padState[i] |= BUTTON_UE;
		////* Ｍボタン
		//if (dijs.rgbButtons[11] & 0x80)	padState[i] |= BUTTON_SHITA;
		////* Ｍボタン
		//if (dijs.rgbButtons[12] & 0x80)	padState[i] |= BUTTON_MIGI;
		////* Ｍボタン
		//if (dijs.rgbButtons[13] & 0x80)	padState[i] |= BUTTON_HIDARI;
